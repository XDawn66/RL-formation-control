...
def make_star_offsets(num_bots, outer_radius=160, inner_radius=75):
    points = []
    ...
    for i in range(10):
        ...
        star_vertices.append(np.array([x, y]))

    # distribute robots along the star outline
    for i in range(num_bots):
        t = i * 10.0 / num_bots

        edge_idx = int(t) % 10
        next_idx = (edge_idx + 1) % 10
        alpha = t - int(t)

        p = (
            (1 - alpha) * star_vertices[edge_idx]
            + alpha * star_vertices[next_idx]
        )
...
class FormationEnv(gym.Env):
    def __init__(self, screen):
        ...
        self.FORMATION_OFFSET = make_star_offsets(self.num_of_bots)
        ...
        self.FORMATION_READY_THRESHOLD = 0.005
